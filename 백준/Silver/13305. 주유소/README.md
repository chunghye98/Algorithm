...
### 내 풀이
아래의 방식으로 풀면 17점으로, 부분 통과만 된다.
1. 현재 도시의 리터당 비용이 다음 도시의 리터당 비용보다 크면 현재 도시에서 많은 기름을 넣는 것은 비용이 높으므로 다음 도시까지 갈 수 있는 리터만 넣는다.
2. 다음 도시의 리터당 이용이 그 다음 도시의 리터당 비용보다 작으면 해당 도시에서 남은 전체 거리만큼 기름을 넣는다.

### 풀이
전체 통과가 되는 방법을 검색하여 찾았다.
1. 리터당 기름 값이 __내림차순__ 일 경우에 주유한다.
   - 만약 주유소별 리터당 가격이 `8, 9, 5, 4, 2, 7, 1` 이라면, 각 도시별 거리에 `8, 8, 5, 4, 2, 2, 1`을 곱해준다(1은 더이상 갈 도로가 없기 때문에 제외한다).
3. 내림차순으로 만든 기름 가격에 각 도시별 거리를 곱하여 더해준다.
