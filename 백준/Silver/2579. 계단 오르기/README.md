...
---
### 풀이
1. 계단은 한번에 한 칸 또는 두 칸씩 오를 수 있다.
2. 연속된 세 개의 계단을 밟을 수는 없다.
3. 마지막 도착 계단은 꼭 밟아야 한다.

연속된 세 개의 계단을 밟을 수 없으므로 계단은 현재 기준으로 dp[i-3]+inputs[i-1] 과 dp[i-2] 중 더 큰 값을 선택해야 한다.
