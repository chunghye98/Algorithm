...
### 풀이
손님의 순서를 적절히 바꿨을 때 팁의 최댓값을 받을 수 있는 방법을 찾는 것이 핵심이다.    
실제 팁은 `생각한 돈 - (받은 등수 - 1)` 이므로 생각한 돈이 큰 사람을 빠른 순서로 바꿔야 최댓값을 받을 수 있다.
1. 리스트에 tip 입력
2. 리스트 내림차순 정렬
3. for문으로 팁 계산하면서 실제 팁잉 0 이하라면 이후의 팁도 음수기 때문에 break

n이 100,000 까지 가능하고, tip[i]가 100,000까지 가능하므로 10,000,000,000 이라는 결과값이 나올 수 있는데 `int`의 범위는 약 21억 까지이므로 결과값의 자료형을 `long`으로 해야 한다.
