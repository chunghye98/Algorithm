...
### 풀이
1. 지도를 전체 탐색하면서 방문하지 않았으면서 값이 1일 때 bfs로 탐색을 한다.
2. bfs로 탐색하면서 도시 수(size)가 0보다 크면 결과 리스트에 담고, count++를 한다.
3. 결과 리스트를 오름차순 정렬한 후 count와 결과 리스트를 출력한다.
