...
### 풀이
n이 100,000까지 가능하므로 O(n^2)까지 나오면 안된다.
1. 입력값을 리스트로 받아 내림차순 정렬한다.
2. `다음 값을 * (처음부터 다음 값 까지의 개수)`한 것이 이전 값보다 크다면 `max`를 갱신한다.
3. `max` 값이 각 로프가 버틸 수 있는 최대 중량이다. 
